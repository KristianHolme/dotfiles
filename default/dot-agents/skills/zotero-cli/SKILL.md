...
> Vendored from `zotero-mcp` (`zotero-mcp install-skill`) into
> `~/dotfiles/default/dot-agents/skills/zotero-cli/`.
...
2. Copy `"$tmp/.agents/skills/zotero-cli/"{SKILL.md,reference.md}` over this skill dir
3. Re-apply the short vendoring note + this reminder block; set `last_refreshed` to today

Do not refresh without asking. If the interval has not elapsed, say nothing about updates.
