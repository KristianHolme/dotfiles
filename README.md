...
| `dotfiles-theme-sync-remote.sh` | Rsync the locally staged Omarchy theme (`~/.local/state/omarchy/current/theme`) to active SSH hosts (ControlMaster only). Applies replica hooks: btop, tmux (status + pane OSC), terminals, gum env, pi, claude, helix, opencode. Hosts with `login_node` apply on that node (VIP rsync, hop for live tmux). No Omarchy clone on the remote. Also run from the `theme-set.d/remote-theme-sync` hook (background, log: `~/.cache/dotfiles/remote-theme-sync.log`). |
